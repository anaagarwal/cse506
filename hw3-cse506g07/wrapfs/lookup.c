...
/*  			EXTRA_CREDIT_2
 * Added New- In dentry *__wrapfs_lookup function we have encrypted
 * filename. the necessary operations are in ifdef comments. otherwise 
 * it runs normally.
 */
...
void free_dentry_private_data(struct dentry *dentry)
{
  ...
  debug_dentry_ops(WRAPFS_SB(dentry->d_sb)->debug_dentry_ops,"Debug_free_dentry_private_data");
#endif
...
#ifdef EXTRA_CREDIT_1
  debug_dentry_ops(WRAPFS_SB(dentry->d_sb)->debug_dentry_ops,"Debug_new_dentry_private_data");
#endif
...
#ifdef EXTRA_CREDIT_1
  debug_inode_ops(WRAPFS_SB(inode->i_sb)->debug_inode_ops,"Debug_wrapfs_inode_test");
#endif	
...
#ifdef EXTRA_CREDIT_1
  debug_inode_ops(WRAPFS_SB(inode->i_sb)->debug_inode_ops,"Debug_wrapfs_inode_get");
#endif
...
#ifdef EXTRA_CREDIT_1
  debug_inode_ops(WRAPFS_SB(sb)->debug_inode_ops,"Debug_wrapfs_iget");
#endif
...
#ifdef EXTRA_CREDIT_1
  debug_dentry_ops(WRAPFS_SB(dentry->d_sb)->debug_dentry_ops,"Debug_wrapfs_interpose");
#endif
...
#ifdef EXTRA_CREDIT_1
  debug_dentry_ops(WRAPFS_SB(dentry->d_sb)->debug_dentry_ops,"err : %d", err);
#endif
				      ...
				      struct path *lower_parent_path)
{
...
#ifdef EXTRA_CREDIT_1
  debug_dentry_ops(WRAPFS_SB(dentry->d_sb)->debug_dentry_ops,"Debug_wrapfs_lookup");
#endif
  ...
  if (encoded_buf != NULL)
    {
      this.name = encoded_buf;
      this.len = strlen(encoded_buf);
