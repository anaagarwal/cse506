 ...
 * two printk enabled in each function
 */
...
static int wrapfs_d_revalidate(struct dentry *dentry, struct nameidata *nd)
{
  ...
  debug_dentry_ops(WRAPFS_SB(dentry->d_sb)->debug_dentry_ops,"Debug_wrapfs_dentry_revalidate");
#endif
...
#ifdef EXTRA_CREDIT_1
  debug_dentry_ops(WRAPFS_SB(dentry->d_sb)->debug_dentry_ops,"err : %d", 0);
#endif
