...
#define MD5_DEFAULT  "d41d8cd98f00b204e9800998ecf8427e"
...
extern ssize_t wrapfs_getxattr(struct dentry *dentry, const char *name,const void *value, size_t size);
...
extern int wrapfs_compute_checksum (struct file*, char *buffer, char *checksum_algorithm);

/* file private data */
struct wrapfs_file_info {
	struct file *lower_file;
	const struct vm_operations_struct *lower_vm_ops;//memory map based ibjects
};
...
//create a link ,delete a link u need a lock
    	spinlock_t lock;	/* protects lower_path */
	struct path lower_path; //dentry,vfsmount
};
...
static inline int wrapfs_set_dflag(struct inode * inode, int d_flag)
{
            WRAPFS_I(inode)->d_flag = d_flag;
	    return WRAPFS_I(inode)->d_flag;
}
static inline int wrapfs_get_dflag(struct inode * inode)
{
                return WRAPFS_I(inode)->d_flag;
}
