...
#include "wrapfs.h"
#define has_integrity_EA "user.has_integrity"
#define integrity_val_EA "user.integrity_val"
			 ...
			 int mode, struct nameidata *nd)
{
	...
	fsstack_copy_inode_size(dir, lower_parent_dentry->d_inode);
        ret =  vfs_getxattr(lower_parent_dentry, has_integrity_EA, (void*) value, (size_t)1);
	if(ret == 1 && value[0] == '1'){
	    ret = vfs_setxattr(lower_dentry, has_integrity_EA, (void *) value, (size_t)1, XATTR_CREATE);
	    ret = vfs_setxattr(lower_dentry, integrity_val_EA, (void *) MD5_DEFAULT, DIGESTLEN*2, XATTR_CREATE);	
	}
...
static int wrapfs_mkdir(struct inode *dir, struct dentry *dentry, int mode)
{
	...
	ret =  vfs_getxattr(lower_parent_dentry, HAS_INTEGRITY, (void*) value, (size_t)1);
	if(ret == 1 && value[0] == '1') {
	    ret = vfs_setxattr(lower_dentry, HAS_INTEGRITY, (void *) value, (size_t)1, XATTR_CREATE);
	}
	else if(ret == 1 && value[0] == '0') { 
	    ret = vfs_setxattr(lower_dentry, HAS_INTEGRITY, (void *) value, (size_t)1, XATTR_CREATE);
	}
